...
- 只读扫描当前 `CODEX_HOME/pets` 或 `~/.codex/pets`；首次先显示一只，多宠物时自动打开选择器。
...
- 提醒来自本地中文模板，会结合宠物身份、久坐分钟和历史听劝次数变化；可选损友、温柔、冷幽默三种语气，避免连续重复。运行期不调用 LLM，也不保存任务正文。
- 单击宠物会庆祝，拖动可移动；悬停后拖右下角或滚轮可在 30%-250% 无级缩放。
- 右键可换宠物、调整阶段与离开电脑灵敏度、设置安静时段、暂停一小时/当天、运行诊断，并生成 1080×1350 今日分享卡。
