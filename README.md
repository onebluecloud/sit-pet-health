...
- 在插件私有数据目录生成一份复制体和七组语义动作：精神、发懒、蔫了、病恹恹、休息、庆祝、拖动。宠物没有明确声明 `tired/sick/rest` 时先用 waiting/idle 安全启动，再由 Codex 图片能力只生成缺失的私有六帧健康动画；联系表通过视觉质检后才原子切换，官方图集始终不动。
...
小红书 RedSkill 版本使用 Release 附件 `rousepet-redskill-v1.2.0.zip`。附件已经包含 Skill、Plugin、Hooks、健康动作补齐流程、脚本和许可，不需要再访问 GitHub，也不会安装 EXE。上传包使用兼容载体保存 YAML 元数据和许可证，避免平台过滤后破坏 Skill 结构。
...
- GitHub Release 文案：[`docs/publish/github-release-v1.2.0.md`](docs/publish/github-release-v1.2.0.md)
...
项目使用 MIT License。用户自己的 Codex 宠物图像仍归各自用户和创作者所有；安装包不包含任何用户宠物源图集。`docs/` 中的 Milkybyte 仅作为项目创作者授权的产品演示截图。

## 开发验证

Windows 回归覆盖健康引擎、多 Codex 会话、离开后返回结算、缺失健康动作生成事务、语义动作 fallback、源文件哈希保护、生命周期、设置页和分享卡。跨平台验证定义保存在 `ci/validate.yml`，覆盖 Windows PowerShell 5、Python/Pillow、Node/JXA 语法与 shell 脚本。
